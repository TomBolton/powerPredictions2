...
def formDataset2( ) :
    ...
    for filename in os.listdir('/home/boltonl/dataScience/cycloAnalytics/cycDataExpl/data') :
        ...
        if filename.endswith('.fit') :
            ...
            fileData = np.zeros( ( power.shape[0], 10 ) )
            ...
            fileData[:,5] = changeHR
            fileData[:,6] = changeSP
            fileData[:,7] = gradient * speed
            fileData[:,8] = np.power( speed, 3 )
            fileData[:,9] = 80 * speed * changeSP + 0.5 * 0.3 * 1.225 * np.power( speed, 3 ) \
                            - 80 * 9.81 * gradient * speed
    ...
    np.save( "cyclingData2", myData )

def loadDataset2() :

    # load data
    myData = np.load('cyclingData2.npy')
